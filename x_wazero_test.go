...
func callWASMFuncWithWazero(t testing.TB, store *wasm.Store, in uint32) uint32 {
	ret, retTypes, err := store.CallFunction(wazeroModName, wasmFuncName, uint64(in))
	if err != nil {
		t.Fatal(err)
	}
	if len(ret) != 1 {
